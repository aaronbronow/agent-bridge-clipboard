...
function readClipboard(): string {
  try {
    if (isWSL) {
      ...
      if (resXclip.status === 0) return resXclip.stdout;

      const resXsel = spawnSync('xsel', ['--clipboard', '--output'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
      if (resXsel.status === 0) return resXsel.stdout;
    }
  } catch (err) {
...
function connect() {
  ...
  ws.on('open', () => {
    ...
    const handshake = createFrame(
      ...
      readClipboard() // Seed with current clipboard value
    );
 ...
 */
function handleServerFrame(frame: ABCFrame) {
